...
class Materia {
    ...
    public function seleccionaMaestro($maestro){
        ...
        $sql02 = "SELECT * FROM usuario WHERE Estatus = 1 AND nivel = 2 ORDER BY Apat ASC";
        ...
        if ($maestro != 0){
            ...
            $nombre = $maestro." ) ";
            $nombre .= mysql_result($result04,0,'Apat');
            $nombre .= " ".mysql_result($result04,0,'Amat');
        ...
        while($field = mysql_fetch_array($result02)){
            $id_maestro = $field['Id'];
            $opcion = utf8_decode($field['Id'].") ".$field['Apat']." ".$field['Amat']." ".$field['Nombre']);
    ...
    public function materiasAsignadas2($id_maestro){
        echo "<div class=table-responsive>";
        echo "<table class=\"table table-striped\">";
        echo "<tr><td colspan=2 align=center><strong>Materias Asignadas</strong></td></tr>";
        $sql01 = "SELECT * FROM maestro_materia WHERE id_maestro = $id_maestro GROUP BY id_materia";
